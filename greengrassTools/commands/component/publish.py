...
def upload_artifacts_s3(component_name, component_version):
    ...
    try:
        ...
        if not utils.dir_exists(project_config["gg_build_component_artifacts_dir"]):
            ...
            raise Exception("""Failed to publish the component '{}' as it is not build.\nBuild the component `greengrass-tools component build` before publishing it.""".format(component_name))
...
def get_latest_component_version(component_name,region,account_num):
    ...
    try:
        ...
        raise Exception("Error while getting the component versions of '{}' in '{}' from the account '{}' during publish.\n{}".format(component_name,region,account_num,e)) 
...
def get_component_version_from_config():
    ...
    try:
        ...
        if project_config["component_version"] == "LATEST":
            logging.debug("Component version set to 'LATEST' in the config file. Calculating next version of the component '{}'".format(project_config["component_name"]))
...
def create_publish_recipe_file(component_name, component_version, parsed_component_recipe):
    ...
    with open(publish_recipe_file, 'w') as prf:
        try:
            logging.debug("Creating component recipe '{}' in '{}'.".format(publish_recipe_file_name, project_config["gg_build_recipes_dir"]))
