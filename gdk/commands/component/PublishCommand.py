...
class PublishCommand(Command):
    def __init__(self, command_args) -> None:
        ...
        self.greengrass_client = Greengrassv2Client(
    ...
    def run(self):
        try:
            ...
            logging.error(
                "Failed to publish new version of the component '{}'".format(
    ...
    def _update_bucket(self):
        if self.arguments.get("bucket"):
            ...
            self.project_config["bucket"] = "{}-{}-{}".format(
                self.project_config["bucket"],
                self.project_config["region"],
                self.project_config["account_number"],
            )

    def _update_options(self):
        if self.arguments.get("options"):
            try:
                self.project_config["options"] = self._read_options(
    ...
    def _read_from_file(self, options):
        logging.debug(
            "Reading options from the json file provided in the publish command"
        )
    ...
    def _update_component_version(self):
        self.project_config[
            "component_version"
        ] = self.get_component_version_from_config()
    ...
    def _update_publish_recipe_file_path(self):
        ...
        self.project_config["publish_recipe_file"] = (
            ...
            .joinpath(publish_recipe_file_name)
            .resolve()
        )

    def _publish_component_version(self, component_name, component_version):
        logging.info(
            f"Publishing the component '{component_name}' with the given project configuration."
        )
        logging.info("Uploading the component built artifacts to s3 bucket.")
        self.upload_artifacts_s3()

        logging.info(
            f"Updating the component recipe {component_name}-{component_version}."
        )
        PublishRecipeTransformer(self.project_config).transform()

        logging.info(
            f"Creating a new greengrass component {component_name}-{component_version}"
        )
    ...
    def upload_artifacts_s3(self) -> None:
        ...
        build_component_artifacts = list(
            self.project_config["gg_build_component_artifacts_dir"].iterdir()
        )
    ...
    def get_next_version(self):
        ...
        try:
            ...
            c_next_patch_version = (
                self.greengrass_client.get_highest_component_version_()
            )
            if not c_next_patch_version:
                logging.info(
                    "No private version of the component '{}' exist in the account. Using '{}' as the next version to create.".format(
                        c_name, fallback_version
                    )
                )
                return fallback_version
            logging.debug(
                "Found latest version '{}' of the component '{}' in the account.".format(
                    c_next_patch_version, c_name
                )
            )
            next_version = utils.get_next_patch_version(c_next_patch_version)
            logging.info(
                "Using '{}' as the next version of the component '{}' to create.".format(
                    next_version, c_name
                )
            )
            return next_version
    ...
    def get_account_number(self):
        ...
        """
        try:
            caller_identity_response = self.service_clients[
                "sts_client"
            ].get_caller_identity()
    ...
    def get_component_version_from_config(self):
        ...
        try:
            ...
            if self.project_config["component_version"] == "NEXT_PATCH":
                logging.debug(
                    "Component version set to 'NEXT_PATCH' in the config file. Calculating next version of the component '{}'".format(
                        self.project_config["component_name"]
                    )
                )
                return self.get_next_version()
            logging.info(
                "Using the version set for the component '{}' in the config file.".format(
                    component_name
                )
            )
            ...
            logging.error(
                "Failed to calculate the version of component '{}' based on the configuration.".format(
                    component_name
                )
            )
            raise (e)
