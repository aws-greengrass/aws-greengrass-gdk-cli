...
from steps.constants import GG_BUILD_DIR, DEFAULT_AWS_REGION
...
def gdk_cli(context):
    ...
    print(
        f"Initializing GDK client{' in debug mode' if debug else ''}{' with instrumented code.' if debug else '.'}"
    )
    # SETUP FIXTURE
    if instrumented:
...
def version_constraint(context, **kwargs):
    ...
    target_version = (
        version_parser(version_option) if not latest_mode else version_option
    )
    ...
    if eq_version and (latest_mode or target_version == eq_version):
        scenario.skip(
            f"Target version {target_version} is not equal to the required version {eq_version}."
        )
    ...
    if lt_version and (latest_mode or target_version >= lt_version):
        scenario.skip(
            f"Target version {target_version} is not lower than required version {lt_version}."
        )
    ...
    if max_version and (latest_mode or target_version > max_version):
        scenario.skip(
            f"Target version {target_version} is higher than required version {max_version}."
        )
    ...
    if not latest_mode and gt_version and target_version <= gt_version:
        scenario.skip(
            f"Target version {target_version} is not greater than required version {gt_version}."
        )
    ...
    if not latest_mode and min_version and target_version < min_version:
        scenario.skip(
            f"Target version {target_version} is lower than required version {min_version}."
        )
...
registry_auto_use_fixtures = [gdk_cli]
...
def after_scenario(context, scenario):
    if "last_component" in context and "last_cli_command_type" in context:
        ...
        if command_type == "component publish":
            ...
            recipes_path = (
                Path(cwd).joinpath(GG_BUILD_DIR).joinpath("recipes").resolve()
            )
            t_utils.clean_up_aws_resources(
                component_name,
                t_utils.get_version_created(recipes_path, component_name),
                DEFAULT_AWS_REGION,
            )
