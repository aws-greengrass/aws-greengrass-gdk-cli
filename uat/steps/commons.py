...
from constants import DEFAULT_AWS_REGION, DEFAULT_S3_BUCKET_PREFIX
...
def get_s3_bucket(context):
    ...
    context.last_s3_bucket = (
        f"{DEFAULT_S3_BUCKET_PREFIX}-{DEFAULT_AWS_REGION}-{aws_account}"
    )
...
def setup_project_from_s3(context, name):
    ...
    s3_client = (
        context.s3_client
        if "s3_client" in context
        else t_utils.create_s3_client(DEFAULT_AWS_REGION)
    )
    s3_client.download_file(bucket, file, str(unique_file_path))
    assert (
        unique_file_path.exists()
    ), f"Couldn't find the downloaded {file} at {unique_file_path}"
