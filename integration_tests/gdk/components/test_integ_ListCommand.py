...
def test_list_run():
    with pytest.raises(Exception) as e:
        parse_args_actions.run_command(
            CLIParser.cli_parser.parse_args(["component", "list", "-d"])
        )
    assert (
        "supply either `--template` or `--repository` as an argument to the list command."
        in e.value.args[0]
    )
...
def test_list_template():
    parse_args_actions.run_command(
        CLIParser.cli_parser.parse_args(["component", "list", "--template"])
    )


def test_list_repository():
    parse_args_actions.run_command(
        CLIParser.cli_parser.parse_args(["component", "list", "--repository"])
    )
...
def test_list_template_failed_request(mocker):
    mock_response = mocker.Mock(
        status_code=404,
        raise_for_status=mocker.Mock(side_effect=HTTPError("some error")),
    )
