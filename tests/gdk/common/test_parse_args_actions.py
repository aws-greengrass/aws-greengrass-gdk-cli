...
def test_run_command_with_mocks(mocker):
    ...
    mock_get_method_from_command = mocker.patch(
        "gdk.common.parse_args_actions.get_method_from_command",
        return_value="_gdk_component_init",
    )
    mock_component_init = mocker.patch(
        "gdk.commands.methods._gdk_component_init", return_value=None
    )
    ...
    mock_get_method_from_command.assert_any_call(
        vars(args_namespace), consts.cli_tool_name, ""
    )
...
def test_run_command_with_invalid_namespace(mocker):
    ...
    args_namespace = argparse.Namespace(
        component="init", lang="python", template="name", **{"gdk": "component"}
    )
    ...
    actions.run_command(args_namespace)
    assert (
        spy_call_action_by_name.call_count == 0
    )  # No method name to call if namespace is invalid
    assert (
        spy_get_method_from_command.call_count == 3
    )  # Recursively called for three times
...
def test_run_command_namespace_as_dict(mocker):
    ...
    spy_get_method_from_command.assert_any_call(
        vars(args_namespace), consts.cli_tool_name, ""
    )
    spy_get_method_from_command.assert_any_call(
        vars(args_namespace), "component", "_gdk"
    )
    spy_get_method_from_command.assert_any_call(
        vars(args_namespace), "init", "_gdk_component"
    )
...
def test_get_method_from_command(mocker):
    ...
    method_name_from_command = actions.get_method_from_command(
        test_d_args, test_command, test_method_name
    )
    spy_get_method_from_command.assert_any_call(
        test_d_args, test_command, test_method_name
    )
