...
def test_gdk(mocker):
    mock_component_init = mocker.patch(
        "gdk.CLIParser.cli_parser.print_help", return_value=None
    )
...
def test_gdk_component_init(mocker):
    mock_component_init = mocker.patch(
        "gdk.commands.component.component.init", return_value=None
    )
...
def test_gdk_component_publish(mocker):
    mock_component_publish = mocker.patch(
        "gdk.commands.component.component.publish", return_value=None
    )
...
def test_gdk_component_list(mocker):
    mock_component_list = mocker.patch(
        "gdk.commands.component.component.list", return_value=None
    )
