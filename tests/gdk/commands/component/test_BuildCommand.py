...
class BuildCommandTest(TestCase):
    ...
    def test_build_run_default(self):
        mock_create_gg_build_directories = self.mocker.patch.object(
            BuildCommand, "create_gg_build_directories"
        )
        mock_default_build_component = self.mocker.patch.object(
            BuildCommand, "default_build_component"
        )
    ...
    def test_default_build_component(self):
        mock_run_build_command = self.mocker.patch.object(
            BuildCommand, "run_build_command"
        )
        ...
        mock_build_info = self.mocker.patch.object(
            BuildCommand, "_get_build_folder_by_build_system", return_value=None
        )
        build = BuildCommand({})
    ...
    def test_default_build_component_error_transform(self):
        ...
        mock_transform = self.mocker.patch.object(
            BuildRecipeTransformer, "transform", side_effect=Error("generating")
        )
    ...
    def test_create_gg_build_directories(self):
        ...
        mock_mkdir.assert_any_call(
            pc["gg_build_recipes_dir"], parents=True, exist_ok=True
        )
        mock_mkdir.assert_any_call(
            pc["gg_build_component_artifacts_dir"], parents=True, exist_ok=True
        )
    ...
    def test_run_build_command_with_error_not_zip(self):
        mock_build_system_zip = self.mocker.patch.object(
            BuildCommand, "_build_system_zip", return_value=None
        )
        mock_subprocess_run = self.mocker.patch(
            "subprocess.run", return_value=None, side_effect=Error("some error")
        )
    ...
    def test_run_build_command_windows(self):
        mock_platform_system = self.mocker.patch(
    ...
    def test_build_system_zip_valid(self):
        ...
        zip_artifacts_path = (
            Path(zip_build_path).joinpath(utils.current_directory.name).resolve()
        )
        mock_build_info = self.mocker.patch.object(
            BuildCommand,
        ...
        mock_clean_dir = self.mocker.patch(
            "gdk.common.utils.clean_dir", return_value=None
        )
        ...
        mock_make_archive.assert_called_with(
            zip_build_file, "zip", root_dir=zip_artifacts_path
        )
        assert mock_get_supported_component_builds.call_count == 1
    ...
    def test_build_system_zip_error_archive(self):
        ...
        mock_copytree = self.mocker.patch("shutil.copytree")
        mock_subprocess_run = self.mocker.patch("subprocess.run", return_value=None)
        mock_make_archive = self.mocker.patch(
            "shutil.make_archive", side_effect=Error("some error")
        )
    ...
    def test_build_system_zip_error_copytree(self):
        ...
        mock_copytree = self.mocker.patch(
            "shutil.copytree", side_effect=Error("some error")
        )
        mock_subprocess_run = self.mocker.patch("subprocess.run", return_value=None)
    ...
    def test_build_system_zip_error_get_build_folder_by_build_system(self):
        ...
        mock_copytree = self.mocker.patch("shutil.copytree")
        mock_subprocess_run = self.mocker.patch("subprocess.run", return_value=None)
        mock_ignore_files_during_zip = self.mocker.patch.object(
            Zip, "get_ignored_file_patterns", return_value=[]
        )
        mock_make_archive = self.mocker.patch("shutil.make_archive")
    ...
    def test_get_build_folder_by_build_system_maven(self):
        ...
        mock_get_build_folders = self.mocker.patch.object(
            BuildCommand, "get_build_folders", return_value=dummy_paths
        )
