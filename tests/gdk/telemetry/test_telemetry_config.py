...
from gdk.telemetry.telemetry_config import (GDK_TELEMETRY_CONFIG_DIR, ConfigKey,
                                            TelemetryConfig)
...
class TestRuntimeConfig(TelemetryTestCase):
    ...
    def test_it_loads_persisted_config_from_file_system(self):
        ...
        config_b = TelemetryConfig(force_create=True)
    ...
    def test_it_can_load_the_config_path_from_an_env_variable(self):
        ...
        config = TelemetryConfig(force_create=True)
        expected_path = Path(temp_dir, "telemetry.json")
