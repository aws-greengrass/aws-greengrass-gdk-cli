...
json_values = {
    ...
    "component_recipe_file": PosixPath("/Users/nukai/workplace/gdk/src/GDK-CLI-Internal/tests/greengrassTools/static/build_command/recipe.json"),
...
def test_update_and_create_recipe_file_manifests_build(mocker):
    ...
    assert publish_component_recipe["Manifests"][0]["Artifacts"][0]["URI"] == f"s3://default/{component_name}/{component_version}/hello_world.py"
...
def test_get_component_version_from_config_exception(mocker):
    mock_get_next_version= mocker.patch("greengrassTools.commands.component.publish.get_next_version", return_value="",side_effect=HTTPError('some error'))
    with pytest.raises(Exception) as e:
...
def test_create_gg_component_exception(mocker):
    mock_client = mocker.patch("boto3.client", return_value=None)
    publish.service_clients = {"greengrass_client":mock_client}
    mock_create_component = mocker.patch("boto3.client.create_component_version", return_value=None, side_effect= HTTPError("error"))
    publish.project_config["publish_recipe_file"] = Path("some-recipe.yaml")
    component_name = "component_name"
    component_version="1.0.0"
...
def test_create_bucket_exception_bucket_owned_not_in_region(mocker):
    bucket="test-bucket"
    region = "region"
    mock_check_in_same_region= mocker.patch("greengrassTools.commands.component.publish.bucket_exists_in_same_region", return_value=False)
    def throw_err(*args, **kwargs):
        ex = boto3.client("s3").exceptions.BucketAlreadyOwnedByYou(
        {"Error": {"Code": "BucketAlreadyOwnedByYou", "Message": "fake message"}}, "CreateBucket"
    )
...
def test_create_bucket_exception_bucket_owned_in_region(mocker):
    bucket="test-bucket"
    region = "region"
    mock_check_in_same_region= mocker.patch("greengrassTools.commands.component.publish.bucket_exists_in_same_region", return_value=True)
    def throw_err(*args, **kwargs):
        ex = boto3.client("s3").exceptions.BucketAlreadyOwnedByYou(
        {"Error": {"Code": "BucketAlreadyOwnedByYou", "Message": "fake message"}}, "CreateBucket"
    )
...
def test_bucket_exists_in_same_region_exists(mocker):
    mock_client = mocker.patch("boto3.client", return_value=None)
    publish.service_clients = {"s3_client":mock_client}
    mock_get_bucket_location = mocker.patch("boto3.client.get_bucket_location", return_value={"LocationConstraint":"us-east-1"})
...
def test_bucket_exists_in_same_region_not_exists(mocker):
    mock_client = mocker.patch("boto3.client", return_value=None)
    publish.service_clients = {"s3_client":mock_client}
    mock_get_bucket_location = mocker.patch("boto3.client.get_bucket_location", return_value={"LocationConstraint":"us-west-2"})
    exists = publish.bucket_exists_in_same_region("bucket","us-east-1")
    assert mock_get_bucket_location.call_count ==1
    assert not exists
